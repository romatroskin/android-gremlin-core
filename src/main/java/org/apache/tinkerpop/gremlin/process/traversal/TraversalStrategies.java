...
public interface TraversalStrategies extends Serializable, Cloneable {
    ...
    public default <T extends TraversalStrategy> Optional<T> getStrategy(final Class<T> traversalStrategyClass) {
        return (Optional) toList().stream().filter(new Predicate<TraversalStrategy<?>>() {
            @Override
            public boolean test(TraversalStrategy<?> s) {
                return traversalStrategyClass.isAssignableFrom(s.getClass());
            }
        }).findAny();
    }
    ...
    public static List<TraversalStrategy<?>> sortStrategies(final List<TraversalStrategy<?>> strategies) {
        ...
        strategies.forEach(new Consumer<TraversalStrategy<?>>() {
            @Override
            public void accept(TraversalStrategy<?> s) {
        ...
        strategies.forEach(new Consumer<TraversalStrategy<?>>() {
            @Override
            public void accept(TraversalStrategy<?> strategy) {
        ...
        List<Class<? extends TraversalStrategy>> unprocessedStrategyClasses = new ArrayList<>(strategies.stream().map(
                new Function<TraversalStrategy<?>, Class<? extends TraversalStrategy>>() {
                    @Override
                    public Class<? extends TraversalStrategy> apply(TraversalStrategy<?> traversalStrategy) {
                        return traversalStrategy.getClass();
                    }
